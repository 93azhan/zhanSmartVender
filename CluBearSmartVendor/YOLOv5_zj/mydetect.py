...
def run(opt,**kwargs):
    ...
    #    model = kwargs["model"]
    #else :
    #    model = DetectMultiBackend(opt.weights, device=select_device(""), dnn=opt.dnn, data=opt.data, fp16=opt.half)
...
# def main(video_path,tmp_path,**kwargs):
    ...
    # parser = parser.parse_args(["--source", video_path, "--tmpdir", tmp_path])
